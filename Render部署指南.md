...
> ⚠️ **重要更正（2026-08-11）**：经实测，Render **Free 实例不支持持久磁盘**（官方明确：Free instances do not support persistent disks）。本指南第 4 步"挂持久磁盘"在 Free 计划下**无法实现**，数据仍会因重启/重部署丢失。
> **若需要数据持久，请用 Railway 方案**：见《Railway持久卷配置指南.md》（Hobby $5/月 + 1GB 卷 $0.25/月，约 $5.25/月，数据永久保存）。
> 若你接受"长期免费但不保证持久 + 勤导 Excel 备份"，本指南第 1–3、5–7 步仍可参考。
